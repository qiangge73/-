...
# LB-START
RootNode = []  # 树的根结点列表 即最底层的所有格点
lb_isFound = False # 迭代结束标志
MARKSTRING = "noN0"
# 定义树结构
class MuxTree:
    def __init__(self):
...
# 将格点插入到树的对应位置
def lb_Insert2Tree(pos, root):
    global lb_isFound
    if not lb_isFound:
        lens = len(root)
        i=0
        while i<lens:
            if ((root[i].data[0] == pos[0]) and (root[i].data[1] == pos[1]) and (root[i].data[2] == pos[2])):
                lb_tempTree = MuxTree()
                lb_tempTree.data[0] = pos[3]
                lb_tempTree.data[1] = pos[4]
                lb_tempTree.data[2] = pos[5]
                root[i].Child.append(lb_tempTree) # 插入结点
                print(root[i].toString())
                lb_isFound = True

            if (len(root[i].Child) > 0):
                lb_Insert2Tree(pos, root[i].Child) # 迭代
            i = i + 1


for lb_item in path2Point:
    lb_isFound = False
    lb_Insert2Tree(lb_item, RootNode)
    if not lb_isFound: # 如果没有找到连接点 则把该结点插入到RootNode树的根结点列表
        lb_tree1 = MuxTree()
        lb_tree2 = MuxTree()

        lb_tree1.data[0] = lb_item[0]
        lb_tree1.data[1] = lb_item[1]
        lb_tree1.data[2] = lb_item[2]
        lb_tree2.data[0] = lb_item[3]
        lb_tree2.data[1] = lb_item[4]
        lb_tree2.data[2] = lb_item[5]

        lb_tree1.Child.append(lb_tree2)
        RootNode.append(lb_tree1)

# 开始------写出所有端点文件
LB_out = open(MARKSTRING +"data81.txt", "w")
LB_out.writelines("最下层点到所有端点的起始点")

nowRoot = MuxTree()

def LB_Print(root):
    global nowRoot
    for lb_items in root:
        if len(lb_items.Child)==0:
            # LB_out.writelines("\n("+str(nowRoot.data[0])+","+str(nowRoot.data[1])+","+str(nowRoot.data[2])+") ("+str(lb_items.data[0])+","+str(lb_items.data[1])+","+str(lb_items.data[2])+")")
            LB_out.writelines("\n"+str(nowRoot.data[0])+","+str(nowRoot.data[1])+","+str(nowRoot.data[2])+","+str(lb_items.data[0])+","+str(lb_items.data[1])+","+str(lb_items.data[2]))
        else:
            LB_Print(lb_items.Child)


for lb_itemOX in RootNode:
    nowRoot = lb_itemOX
    LB_Print(lb_itemOX.Child)
...
LB_out_1 = open(MARKSTRING +"data-highest-81.txt", "w")
LB_out_1.writelines("最下层点到最高格点")
...
def LB_Print2(root,level):
    global LB_Root
    global LB_Treetop
    global maxLevel
    global LB_TreetopList
    for lb_items in root:
        if len(lb_items.Child)==0 :
            if level == maxLevel:
                LB_TreetopList.append(lb_items)
            if level > maxLevel:
                LB_TreetopList =[]
                LB_TreetopList.append(lb_items)
                maxLevel = level
        else:
            LB_Print2(lb_items.Child,level+1)
...
for lb_itemOX in RootNode:
    ...
    maxLevel = 0
    LB_TreetopList = []
    LB_Print2(lb_itemOX.Child,0)
    for lb_itemOC in LB_TreetopList:
        # LB_out_1.writelines(
        #     "\n(" + str(LB_Root.data[0]) + "," + str(LB_Root.data[1]) + "," + str(LB_Root.data[2]) + ") (" + str(
        #         lb_itemOC.data[0]) + "," + str(lb_itemOC.data[1]) + "," + str(lb_itemOC.data[2]) + ")")
        LB_out_1.writelines(
            "\n" + str(LB_Root.data[0]) + "," + str(LB_Root.data[1]) + "," + str(LB_Root.data[2]) + "," + str(
                lb_itemOC.data[0]) + "," + str(lb_itemOC.data[1]) + "," + str(lb_itemOC.data[2]))
LB_out_1.close()
...
LB_out_2 = open(MARKSTRING +"data-tree-81.txt","w")

def OutputTreeFile(tree,level) :
    for m in tree:
        i = 0
        LB_out_2.writelines("\n")
        while i<level:
            LB_out_2.writelines(" ")
            i = i+1
        LB_out_2.writelines("(" + str(m.data[0]) + "," + str(m.data[1]) + "," + str(m.data[2]) + ")")

        if len(m.Child) > 0:
            OutputTreeFile(m.Child,level+1)

OutputTreeFile(RootNode,0)
