...
LB_out = open("data-highest-82.txt","w")

LB_out.writelines("最高格点")
LB_N0Point = []
LB_isFound = False

for item_01 in N0path2Point:
    if item_01[0] == 0:
        LB_isFound = False
        for item_02 in LB_N0Point:
            if (item_01[0] == item_02[0]) and (item_01[1] == item_02[1]) and (item_01[2] == item_02[2]) :
                LB_isFound = True
        if not LB_isFound:
            LB_N0Point.append([item_01[0],item_01[1],item_01[2]])


LB_LinkablePoint = []

count_i=1
count_n = len(LB_N0Point)
for item_01 in LB_N0Point:
    LB_LinkablePoint = []
    LB_LinkablePoint.append(item_01)
    for item_02 in N0path2Point:
        for item_03 in LB_LinkablePoint:
            if (item_02[0] == item_03[0]) and (item_02[1] == item_03[1]) and (item_02[2] == item_03[2]):
                LB_LinkablePoint.append([item_02[3],item_02[4],item_02[5]])
                break
    print(str(item_01) + "关系点寻找完毕.")
    LB_HighestPoint = []
    LB_height = -1
    for item_04 in LB_LinkablePoint:
        if item_04[0] == LB_height:
            LB_HighestPoint.append(item_04)
        elif item_04[0] > LB_height:
            LB_HighestPoint = []
            LB_HighestPoint.append(item_04)
            LB_height = item_04[0]
    print(str(item_01) + "最高格点寻找完毕，共"+str(len(LB_HighestPoint))+"个")
    for item_05 in LB_HighestPoint:
        LB_out.writelines("\n"+str(item_01[0])+","+str(item_01[1])+","+str(item_01[2])+","+str(item_05[0])+","+str(item_05[1])+","+str(item_05[2]))
    count_i = count_i + 1
    print("进度："+str(count_i)+"/"+str(count_n))
